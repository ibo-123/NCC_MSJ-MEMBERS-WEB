...
interface DashboardData {
  overview?: {
    ...
    achievements?: { total?: number; verified?: number };
  };
  charts?: {
    attendanceTrend?: { date: string; rate: number }[];
    userDepartments?: { _id: string; count: number }[];
  };
  leaderboard?: { name: string; studentId: string; points: number }[];
  recentActivity?: {
    newUsers?: {
      _id: string;
      name: string;
  ...
  quickStats?: { newUsersToday?: number };
}
...
export default function AdminDashboard() {
  ...
  const {
    overview = { users: {}, courses: {}, achievements: {} },
    charts = { attendanceTrend: [], userDepartments: [] },
    leaderboard = [],
    recentActivity = { newUsers: [], upcomingEvents: [] },
    quickStats = {},
  } = dashboardData || {};

  // Safe access to user counts with defaults
        ...
        <div className="max-w-7xl mx-auto">
            ...
            <div className="bg-gray-900/70 border border-green-900/30 rounded-2xl p-6">
              ...
              <div className="space-y-4">
                {charts?.userDepartments?.map((dept, i) => {
                  const percentage =
                    totalUsers > 0 ? (dept.count / totalUsers) * 100 : 0;
                  return (
                    <div key={i}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-gray-900/70 border border-green-900/30 rounded-2xl p-6">
              ...
              <div className="space-y-4">
                {recentActivity?.upcomingEvents?.map((event) => (
                  <div
                    key={event._id}
                    className="flex items-center justify-between"
                  >
